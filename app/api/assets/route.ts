  ...
  try {
    ...
    if (audit) {
      return Response.json({
        ...
        total: rows.length,
        source: "supabase",
        audit: rows.map(({ id, category, subcategory, filename, orientation, framing, activity, mood, colors, use_count, source_type, persona_id }) => ({ id, category, subcategory, filename, orientation, framing, activity, mood, colors, use_count, source_type, persona_id })),
      });
    }
