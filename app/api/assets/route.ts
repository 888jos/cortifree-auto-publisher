...
export async function GET() {
  try {
    const r = await supabase(`assets?workspace_id=eq.${CORTIFREE_WORKSPACE_ID}&storage_bucket=eq.cortifree-assets&select=id,category,subcategory,filename,orientation,framing,activity,mood,colors,public_url,use_count,source_type,persona_id,last_used_at&enabled=eq.true&order=category.asc,filename.asc&limit=1000`);
