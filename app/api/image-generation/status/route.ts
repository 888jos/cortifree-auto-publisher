...
export async function GET() {
  ...
  try {
    const response = await supabase("image_generation_usage?workspace_id=eq." + CORTIFREE_WORKSPACE_ID + "&created_at=gte." + encodeURIComponent(monthStart) + "&select=images_generated,estimated_cost_usd");
