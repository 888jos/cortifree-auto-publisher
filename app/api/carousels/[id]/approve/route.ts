...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const response = await supabase(`carousels?id=eq.${encodeURIComponent(id)}&select=id,account_id,spec,status,topic,angle,caption,language,cta_type&limit=1`);
    ...
    const update = await supabase(`carousels?id=eq.${encodeURIComponent(id)}`, { method: "PATCH", body: JSON.stringify({ status: nextStatus, spec: updatedSpec, updated_at: new Date().toISOString() }) });
