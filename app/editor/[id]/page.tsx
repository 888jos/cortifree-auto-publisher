...
export default function Editor({params}:{params:Promise<{id:string}>}){
 ...
 const generated=carousel?.spec?.generated_slides||[];
 function renderedAt(index:number){
  const candidate=generated[index]||{},position=Number(candidate.position||index+1),list=Array.isArray(carousel?.spec?.rendered_slides)?carousel.spec.rendered_slides:[];
  return list.find((item:any)=>Number(item?.position)===position)||list[index]||{};
 }
 ...
 useEffect(()=>{if(!editingText)return;const el=editingText==="headline"?headlineEditRef.current:bodyEditRef.current;if(!el)return;el.textContent=String(editingText==="headline"?headline:body);requestAnimationFrame(()=>{el.focus();const range=document.createRange();range.selectNodeContents(el);range.collapse(false);const selection=window.getSelection();selection?.removeAllRanges();selection?.addRange(range)})},[editingText,key]);
 const renderedIds=Array.isArray(renderedFallback?.assetIds)?renderedFallback.assetIds:(renderedFallback?.assetId!=null?[renderedFallback.assetId]:[]);
 const sourceIds=Array.isArray(slide.render_metadata?.asset_ids)&&slide.render_metadata.asset_ids.length?slide.render_metadata.asset_ids:renderedIds.length?renderedIds:[slide.asset_id];
 const originalIds=sourceIds.filter(Boolean),assetIds=ov.assetIds||originalIds,storedSlots=Array.isArray(storedGeometry?.imageSlots)?storedGeometry.imageSlots:[],slots=ov.imageSlots||(storedSlots.length?storedSlots:defaultSlots(layout,isHook,assetIds.length));
 ...
 const finalRenderUrl=slide.rendered_url||renderedFallback?.url||"";
 function assignedIds(index:number){const s=slides[index]||{},g=generated[index]||{},k=String(g.position||s.position||index+1),o=overrides[k]||{},r=renderedAt(index),renderIds=Array.isArray(r?.assetIds)?r.assetIds:(r?.assetId!=null?[r.assetId]:[]),source=Array.isArray(s.render_metadata?.asset_ids)&&s.render_metadata.asset_ids.length?s.render_metadata.asset_ids:renderIds.length?renderIds:[s.asset_id],original=source.filter(Boolean);return (o.assetIds||original) as Array<string|number>}
 function slideHealth(index:number){const s=slides[index]||{},g=generated[index]||{},k=String(g.position||s.position||index+1),o=overrides[k]||{},ids=assignedIds(index),headlineValue=String(o.headline??g.headline??s.headline??""),bodyValue=String(o.body??g.body??s.body??""),role=String(g.role||"").toUpperCase(),hook=index===0||role==="HOOK",assetType=String(g.assetType??g.asset_type??"").toLowerCase(),expected=defaultSlots(layout,hook,Math.max(1,ids.length)).length,issues:Array<{level:"error"|"warn";label:string;detail?:string;selection?:"headline"|"body"|number}>=[];if(!headlineValue.trim())issues.push({level:"error",label:"Titre manquant",selection:"headline"});if(/\b(lorem|placeholder|todo|tbd|undefined|null)\b/i.test(headlineValue+" "+bodyValue))issues.push({level:"error",label:"Texte temporaire détecté",selection:"headline"});if(headlineValue.length>90)issues.push({level:"warn",label:"Titre trop long",detail:headlineValue.length+" chars",selection:"headline"});if(bodyValue.length>280)issues.push({level:"warn",label:"Texte trop long",detail:bodyValue.length+" chars",selection:"body"});if(assetType!=="text_only"&&ids.length<expected)issues.push({level:"warn",label:"Images manquantes",detail:(expected-ids.length)+" image"+(expected-ids.length>1?"s":"")+" à ajouter pour ce modèle",selection:Math.min(ids.length,expected-1)});const resolved=ids.map(aid=>assets.find(a=>String(a.id)===String(aid)));resolved.forEach((a,i)=>{if(!a)issues.push({level:"error",label:"Image source introuvable",detail:"Choisir une image de remplacement",selection:i})});const personaRequired=assetType==="persona"||(hook&&assetType!=="app_screenshot");if(personaRequired&&!resolved.some(a=>a?.source_type==="persona_generated"&&(!carousel?.persona_id||a.persona_id===carousel.persona_id)))issues.push({level:"error",label:"Photo du persona requise",detail:hook?"Le hook doit utiliser le persona du carrousel":"Cette slide doit utiliser le persona du carrousel",selection:0});if(!(s.rendered_url||carousel?.spec?.rendered_slides?.[index]?.url))issues.push({level:"warn",label:"Rendu final absent"});return {level:issues.some(x=>x.level==="error")?"error":issues.length?"warn":"ok",issues}}
 ...
 function commitInline(kind:"headline"|"body",event:React.FocusEvent<HTMLSpanElement>){if(isLocked(kind)){setEditingText(null);return}const value=(event.currentTarget.textContent||"").replace(/\n{3,}/g,"\n\n").trim();const current=kind==="headline"?String(headline):String(body);if(value!==current)setOv(kind==="headline"?{headline:value}:{body:value});setEditingText(null)}
 ...
 const canvasWidth=zoomMode==="fit"?"min(620px, calc((100vh - 170px) * .8), calc(100% - 24px))":`${1080*(Number(zoomMode)/100)}px`;
 return <main className="ce-shell" tabIndex={-1} onKeyDown={e=>{
  ...
  <header className="ce-top">
   ...
   <details className="ce-more"><summary aria-label="Plus d’actions">•••</summary><div className="ce-more-menu">
    <button onClick={()=>{setTemplateMode(true);setPreviewMode(false);setDiagnoseMode(false);setHistoryMode(false)}}>Modèle</button>
    <button onClick={()=>{void loadVersions();setHistoryMode(true);setPreviewMode(false);setDiagnoseMode(false);setTemplateMode(false)}}>Historique</button>
    <button onClick={()=>void saveNow()}>Sauvegarder une version</button>
   </div></details>
   <button className={"primary ce-render-button "+(renderOutdated?"stale":"")} onClick={()=>{if(blockingCount){setDiagnoseMode(true);setPreviewMode(false);setBusy(blockingCount+" problème"+(blockingCount>1?"s":"")+" bloquant le rendu")}else void save(true)}}><span>Mettre à jour le rendu</span>{renderOutdated&&<small>Modifications non rendues</small>}</button>
  </header>
  ...
  <aside className="ce-slides">
   ...
   {generated.map((s:any,i:number)=>{
    ...
    const thumb=slides[i]?.rendered_url||renderedAt(i)?.url||fallback||"";
    ...
    return <div key={s.position??i} className={"ce-slide-card "+(i===active?"active ":"")+(slideDragIndex===i?"dragging ":"")} draggable={!protectedSlide&&!structureBusy} onDragStart={e=>{if(protectedSlide){e.preventDefault();return}setSlideDragIndex(i);e.dataTransfer.effectAllowed="move"}} onDragOver={e=>{if(slideDragIndex!==null&&!protectedSlide){e.preventDefault();e.dataTransfer.dropEffect="move"}}} onDrop={e=>{e.preventDefault();if(slideDragIndex!==null&&!protectedSlide&&slideDragIndex!==i)void structureAction({action:"reorder",fromIndex:slideDragIndex,toIndex:i})}} onDragEnd={()=>setSlideDragIndex(null)}>
      <button className="ce-slide-main" onClick={()=>{setActive(i);setSelection("headline");setEditingText(null);setPreviewMode(false)}}>
       <div className="ce-thumb-wrap">{thumb?<img src={thumb} alt={"Slide "+(i+1)}/>:<div className="ce-thumb-empty">No render</div>}<span className={"ce-health "+health.level}>{health.level==="ok"?"✓":health.level==="error"?"!":"⚠"}</span>{protectedSlide&&<span className="ce-structure-lock" title="Protected format slide">◆</span>}{renderOutdated&&thumb&&<span className="ce-stale-thumb">Ancien rendu</span>}</div>
   ...
   {previewMode?
    ...
    <>
     <div className="ce-contextbar">
      <div className="ce-context-meta"><strong>Slide {active+1}</strong><span>· {String(gen.role||"slide").toLowerCase()}</span></div>
      {activeHealth.level!=="ok"&&<button className={"ce-health-shortcut "+activeHealth.level} onClick={()=>setDiagnoseMode(true)}>{activeHealth.issues.length} à vérifier</button>}
      <div className="ce-context-tools">
       <label className="ce-zoom-control"><span>Zoom</span><select value={zoomMode} onChange={e=>setZoomMode(e.target.value as "fit"|"50"|"75"|"100")}><option value="fit">Fit</option><option value="50">50%</option><option value="75">75%</option><option value="100">100%</option></select></label>
       <button className={"ce-snap-toggle "+(editorState.snap===false?"":"active")} onClick={toggleSnap}>Guides</button>
      </div>
     </div>
     <div ref={canvasRef} className={"ce-canvas layout-"+layout+(slotAssets.length<slots.length?" has-missing":"")} style={{width:canvasWidth}}>
      ...
      {slots.map((frame:Frame,i:number)=>{
       ...
       return <div key={i} className={"ce-image-frame "+(selection===i?"sel ":"")+(asset?"":"ce-missing-slot ")+(locked?"locked":"")} style={{left:frame.x*canvasScale,top:frame.y*canvasScale,width:frame.width*canvasScale,height:frame.height*canvasScale}} onClick={()=>setSelection(i)} onPointerDown={e=>{setSelection(i);down(e,"move",i)}} onPointerMove={move} onPointerUp={endDrag} onPointerCancel={endDrag}>
        {asset?<img draggable={false} src={asset.public_url} style={{transform:`scale(${frame.zoom||1})`,objectPosition:`${frame.cropX??50}% ${frame.cropY??50}%`}}/>:<span className="ce-empty-slot-label"><b>+ Choisir une image</b><small>Slot {i+1}</small></span>}
      ...
      {!isHidden("headline")&&<div className={"ce-text "+(selection==="headline"?"sel ":"")+(editingText==="headline"?"editing ":"")+(isLocked("headline")?"locked":"")} onClick={()=>setSelection("headline")} onDoubleClick={e=>{e.stopPropagation();setSelection("headline");if(!isLocked("headline"))setEditingText("headline")}} onPointerDown={e=>{if(editingText!=="headline")down(e,"move","headline");else e.stopPropagation()}} onPointerMove={move} onPointerUp={endDrag} onPointerCancel={endDrag} style={{left:Number(text.headlineX??text.x??90)*canvasScale,top:Number(text.headlineY??text.y??700)*canvasScale,width:Number(text.width||850)*canvasScale,fontSize:Number(text.headlineSize||54)*canvasScale,color:text.editorHeadlineColor||text.headlineColor||"#fff",fontFamily:text.fontFamily||"TikTok Sans",textAlign:text.align||"left",fontWeight:text.headlineWeight||700}}><span ref={headlineEditRef} className="ce-inline-text" contentEditable={editingText==="headline"&&!isLocked("headline")} suppressContentEditableWarning onBlur={e=>commitInline("headline",e)}>{editingText==="headline"?null:headline}</span>{selection==="headline"&&editingText!=="headline"&&!isLocked("headline")&&<i className="ce-handle" onPointerDown={e=>down(e,"resize","headline")} onPointerMove={move}/>} {isLocked("headline")&&<span className="ce-lock-badge">Locked</span>}</div>}
      {body&&!isHidden("body")&&<div className={"ce-text body "+(selection==="body"?"sel ":"")+(editingText==="body"?"editing ":"")+(isLocked("body")?"locked":"")} onClick={()=>setSelection("body")} onDoubleClick={e=>{e.stopPropagation();setSelection("body");if(!isLocked("body"))setEditingText("body")}} onPointerDown={e=>{if(editingText!=="body")down(e,"move","body");else e.stopPropagation()}} onPointerMove={move} onPointerUp={endDrag} onPointerCancel={endDrag} style={{left:Number(text.bodyX??text.x??90)*canvasScale,top:Number(text.bodyY||900)*canvasScale,width:Number(text.width||850)*canvasScale,fontSize:Number(text.bodySize||28)*canvasScale,color:text.editorBodyColor||text.bodyColor||"#fff",fontFamily:text.fontFamily||"TikTok Sans",textAlign:text.align||"left",fontWeight:text.bodyWeight||500}}><span ref={bodyEditRef} className="ce-inline-text" contentEditable={editingText==="body"&&!isLocked("body")} suppressContentEditableWarning onBlur={e=>commitInline("body",e)}>{editingText==="body"?null:body}</span>{selection==="body"&&editingText!=="body"&&!isLocked("body")&&<i className="ce-handle" onPointerDown={e=>down(e,"resize","body")} onPointerMove={move}/>} {isLocked("body")&&<span className="ce-lock-badge">Locked</span>}</div>}
     </div>
    </>
   }
  ...
  <aside className="ce-props">
   ...
   diagnoseMode?
    <div className="ce-diagnostics">
     ...
     {diagnostics.map((d:any,i:number)=>d.issues.length?<div className="ce-diagnostic-group" key={i}><h3>Slide {i+1} · {String(generated[i]?.role||"slide")}</h3>{d.issues.map((issue:any,j:number)=><button key={j} className={"ce-finding "+issue.level} onClick={()=>{setActive(i);setSelection(issue.selection??"headline");setDiagnoseMode(false);setPreviewMode(false)}}><span>{issue.level==="error"?"!":"⚠"}</span><div><b>{issue.label}</b>{issue.detail&&<small>{issue.detail}</small>}<em>Corriger →</em></div></button>)}</div>:null)}
    </div>:
    <>
     <details className="ce-layer-details"><summary><span>Calques</span><small>{1+(body?1:0)+slots.length} éléments</small></summary><div className="ce-layerlist">
     ...
     <div className="ce-layer-actions"><button onClick={copySelected}>Copier <kbd>⌘C</kbd></button><button disabled={!clipboard} onClick={pasteSelected}>Coller <kbd>⌘V</kbd></button></div></details>
     ...
     {typeof selection!=="number"?
      <div className={"ce-panel "+(isLocked(selection)?"locked":"")}>
       ...
       <details className="ce-advanced"><summary>Position & réglages avancés</summary>
      ...
      <div className={"ce-panel "+(isLocked(selection)?"locked":"")}>
       ...
       {selectedAsset&&<details className="ce-asset-info"><summary>Infos asset</summary><div className="ce-provenance"><span><b>Source</b>{selectedAsset.source_type||"—"}</span><span><b>Persona</b>{selectedAsset.persona_id||"—"}</span><span><b>Utilisations</b>{selectedAsset.use_count??0}</span><span><b>Drive</b>{selectedAsset.drive_file_id?"lié":"—"}</span>{Boolean(selectedAsset.metadata?.generation_job_id)&&<span className="wide-meta"><b>Generation job</b>{String(selectedAsset.metadata?.generation_job_id)}</span>}</div></details>}
       <details className="ce-advanced"><summary>Position & taille</summary>
