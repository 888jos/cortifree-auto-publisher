...
export default function CarouselEditor({params}:{params:Promise<{id:string}>}) {
 ...
 const [scene,setScene]=useState(""); const drag=useRef<any>(null);
 const [history,setHistory]=useState<Record<string,Override>[]>([]); const [future,setFuture]=useState<Record<string,Override>[]>([]);
 ...
 const sourceAsset=useMemo(()=>assets.find(a=>String(a.id)===String(ov.assetId??slide.asset_id)),[assets,ov.assetId,slide.asset_id]);
 const preview=sourceAsset?.public_url || slide.rendered_url || carousel?.spec?.rendered_slides?.[active]?.url;
 function commit(mutator:(current:Record<string,Override>)=>Record<string,Override>){setOverrides(current=>{setHistory(h=>[...h.slice(-39),structuredClone(current)]);setFuture([]);return mutator(current)});setDirty(true)}
 function patch(next:Override){commit(o=>({...o,[key]:{...(o[key]||{}),...next}}))}
 function livePatch(next:Override){setOverrides(o=>({...o,[key]:{...(o[key]||{}),...next}}));setDirty(true)}
 function undo(){setHistory(h=>{if(!h.length)return h;const previous=h[h.length-1]!;setFuture(f=>[structuredClone(overrides),...f].slice(0,40));setOverrides(previous);setDirty(true);return h.slice(0,-1)})}
 function redo(){setFuture(f=>{if(!f.length)return f;const next=f[0]!;setHistory(h=>[...h.slice(-39),structuredClone(overrides)]);setOverrides(next);setDirty(true);return f.slice(1)})}
 function textPatch(next:Record<string,any>,live=false){(live?livePatch:patch)({text:{...(ov.text||{}),...next}})}
 function imagePatch(next:Record<string,any>,live=false){(live?livePatch:patch)({image:{...(ov.image||{}),...next}})}
 function pointerDown(e:React.PointerEvent,kind:"move"|"resize"="move"){if(selected==="image")return;e.stopPropagation();setHistory(h=>[...h.slice(-39),structuredClone(overrides)]);setFuture([]);drag.current={kind,x:e.clientX,y:e.clientY,startX:Number(text.x||82),startY:Number(selected==="headline"?(text.headlineY??text.y??810):(text.bodyY??980)),startW:Number(text.width||916),startSize:Number(selected==="headline"?text.headlineSize||52:text.bodySize||28)};(e.currentTarget as HTMLElement).setPointerCapture(e.pointerId)}
 function pointerMove(e:React.PointerEvent){if(!drag.current)return;const dx=(e.clientX-drag.current.x)*2,dy=(e.clientY-drag.current.y)*2;if(drag.current.kind==="resize"){const width=Math.max(120,Math.min(1080-drag.current.startX,drag.current.startW+dx));const size=Math.max(12,Math.min(140,drag.current.startSize+dy*.18));textPatch(selected==="headline"?{width:Math.round(width),headlineSize:Math.round(size)}:{width:Math.round(width),bodySize:Math.round(size)},true)}else{textPatch(selected==="headline"?{x:Math.round(drag.current.startX+dx),headlineY:Math.round(drag.current.startY+dy)}:{x:Math.round(drag.current.startX+dx),bodyY:Math.round(drag.current.startY+dy)},true)}}
 function pointerUp(){drag.current=null}
 function imagePointerDown(e:React.PointerEvent,kind:"move"|"resize"){e.stopPropagation();setSelected("image");setHistory(h=>[...h.slice(-39),structuredClone(overrides)]);setFuture([]);drag.current={kind:"image-"+kind,x:e.clientX,y:e.clientY,startX:Number(image.x||0),startY:Number(image.y||0),startW:Number(image.width||1080),startH:Number(image.height||1350)};(e.currentTarget as HTMLElement).setPointerCapture(e.pointerId)}
 function imagePointerMove(e:React.PointerEvent){if(!drag.current||!String(drag.current.kind).startsWith("image-"))return;const dx=(e.clientX-drag.current.x)*2,dy=(e.clientY-drag.current.y)*2;if(drag.current.kind==="image-resize")imagePatch({width:Math.max(80,Math.round(drag.current.startW+dx)),height:Math.max(80,Math.round(drag.current.startH+dy))},true);else imagePatch({x:Math.round(drag.current.startX+dx),y:Math.round(drag.current.startY+dy)},true)}
 function keyboard(e:React.KeyboardEvent){if((e.metaKey||e.ctrlKey)&&e.key.toLowerCase()==="z"){e.preventDefault();e.shiftKey?redo():undo()}if((e.metaKey||e.ctrlKey)&&e.key.toLowerCase()==="y"){e.preventDefault();redo()}}
 ...
 return <main className="ce-shell" tabIndex={-1} onKeyDown={keyboard}>
  <header className="ce-top"><a href="/">← CortiFree</a><strong>Carousel Studio</strong><button disabled={!history.length} onClick={undo}>↶ Undo</button><button disabled={!future.length} onClick={redo}>↷ Redo</button><span>{dirty?"Unsaved changes":"Saved"}</span><button onClick={()=>save(false)}>Save</button><button className="primary" onClick={()=>save(true)}>Save & render</button></header>
  ...
  <section className="ce-work">
   <div className="ce-canvas">
    {preview&&<div className={"ce-image-frame "+(selected==="image"?"sel":"")} onClick={()=>setSelected("image")} onPointerDown={e=>imagePointerDown(e,"move")} onPointerMove={imagePointerMove} onPointerUp={pointerUp} style={{left:(Number(image.x||0)/2)+"px",top:(Number(image.y||0)/2)+"px",width:(Number(image.width||1080)/2)+"px",height:(Number(image.height||1350)/2)+"px"}}><img draggable={false} src={preview} style={{transform:`scale(${Number(image.zoom||1)})`,objectPosition:`${Number(image.cropX??50)}% ${Number(image.cropY??50)}%`}}/>{selected==="image"&&<i className="ce-handle" onPointerDown={e=>imagePointerDown(e,"resize")} onPointerMove={imagePointerMove} onPointerUp={pointerUp}/>}</div>}
    <div className={"ce-text headline "+(selected==="headline"?"sel":"")} onClick={()=>setSelected("headline")} onPointerDown={pointerDown} onPointerMove={pointerMove} onPointerUp={pointerUp} style={{left:(Number(text.x||82)/2)+"px",top:(Number(text.headlineY??text.y??810)/2)+"px",width:(Number(text.width||916)/2)+"px",fontSize:(Number(text.headlineSize||52)/2)+"px",color:text.headlineColor||"#fff",fontFamily:text.fontFamily||"TikTok Sans",textAlign:text.align||"left",fontWeight:text.headlineWeight||700}}>{headline}{selected==="headline"&&<i className="ce-handle" onPointerDown={e=>pointerDown(e,"resize")} onPointerMove={pointerMove} onPointerUp={pointerUp}/>}</div>
    {body&&<div className={"ce-text body "+(selected==="body"?"sel":"")} onClick={()=>setSelected("body")} onPointerDown={pointerDown} onPointerMove={pointerMove} onPointerUp={pointerUp} style={{left:(Number(text.x||82)/2)+"px",top:(Number(text.bodyY||980)/2)+"px",width:(Number(text.width||916)/2)+"px",fontSize:(Number(text.bodySize||28)/2)+"px",color:text.bodyColor||"#fff",fontFamily:text.fontFamily||"TikTok Sans",textAlign:text.align||"left",fontWeight:text.bodyWeight||500}}>{body}{selected==="body"&&<i className="ce-handle" onPointerDown={e=>pointerDown(e,"resize")} onPointerMove={pointerMove} onPointerUp={pointerUp}/>}</div>}
  ...
  <aside className="ce-props">
   ...
   {selected!=="image"?<div className="ce-panel">
    ...
    <label>Zoom <input type="range" min="1" max="4" step=".05" value={image.zoom||1} onChange={e=>imagePatch({zoom:+e.target.value})}/></label>
    <div className="ce-row"><label>Crop X<input type="range" min="0" max="100" value={image.cropX??50} onChange={e=>imagePatch({cropX:+e.target.value})}/></label><label>Crop Y<input type="range" min="0" max="100" value={image.cropY??50} onChange={e=>imagePatch({cropY:+e.target.value})}/></label></div>
    <button onClick={()=>patch({assetId:undefined,image:{}})}>Reset image</button><h3>Replace from library</h3><input placeholder="Search assets…" value={assetSearch} onChange={e=>setAssetSearch(e.target.value)}/><div className="ce-assets">{assets.filter(a=>!assetSearch||a.filename.toLowerCase().includes(assetSearch.toLowerCase())||String(a.source_type||"").includes(assetSearch.toLowerCase())).slice(0,120).map(a=><button key={a.id} onClick={()=>patch({assetId:a.id})}><img src={a.public_url}/></button>)}</div>
