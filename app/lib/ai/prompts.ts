...
- Use only the canonical CortiFree formats F01-F09. Vary slide roles and copy rhythm based on the selected format.
- FORMAT RULES:
  ...
  - F05_INTERACTIVE_CHECKLIST: interactive/second-person with a locked self-audit layout. Cover is one short problem/question hook plus one short instruction. Every body slide asks exactly ONE concrete question in the headline and body MUST contain 2-4 concise choices separated by " | ". Choices should be things the viewer can genuinely recognize in their routine, not vague advice. Final slide gives one short takeaway/save CTA and does not use choices.
  ...
  - F09_LIFESTYLE_3STACK: native lifestyle/health carousel matching organic TikTok photo dumps. Cover uses ONE full-screen aspirational lifestyle photo with a short 3-7 word hook and tiny optional context line. Every body/final slide contains exactly ONE concrete habit or behavior, with exactly THREE cohesive but distinct horizontal lifestyle photos stacked vertically edge-to-edge. Headline is 2-5 casual words, lowercase preferred. Body is 1-3 short practical lines, max ~28 words, written like a creator caption rather than an explainer. Put the text over the MIDDLE image in a readable quiet area. The three images must depict the same habit from three different scenes/angles (person/action, environment/detail, POV/food/object). No cards, borders, infographic panels, fake UI, labels baked into images, or generic AI collage. Reuse existing concepts freely: BEAUTY_WELLNESS_HACKS is primary, but THINGS_CHANGED, GLOW_UP, MORNING_ROUTINE and NIGHT_ROUTINE are valid editorial concepts for this same format.
...
- interactive-checklist uses one contextual full-screen photo with a rounded cream panel rendered programmatically. Cover panel sits low over the image. Body panel contains one question plus 2-4 programmatic empty checkboxes; supply choices only as pipe-separated body copy ("choice one | choice two | choice three"). Final panel is a takeaway/CTA. Never ask the image model to draw checkboxes, text, UI, cards, labels or buttons.
