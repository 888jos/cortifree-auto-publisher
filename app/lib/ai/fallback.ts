...
const enRankingSteps = [
  ["F · CORTISOL DETOX SUPPLEMENTS", "Expensive does not mean useful. If my sleep and daily routine are chaos, a trendy powder is not the part I would obsess over first."],
  ["D · PERFECT 10-STEP ROUTINES", "Cute online, exhausting in real life. The more steps I add, the more likely I am to quit the whole thing three days later."],
  ["C · MAGNESIUM BEFORE BED", "I like it as a small ritual, not a magic fix. It can sit in the routine, but I would not build the routine around it."],
  ["B · 10 MINUTES OUTSIDE", "Low effort and actually realistic on a busy day. I would take this over another complicated wellness rule I need a spreadsheet to remember."],
  ["A · PHONE OUT OF BED", "Annoying at first, then weirdly freeing. Removing the easiest source of midnight scrolling makes my evening feel much less chaotic."],
  ["S · CONSISTENT SLEEP", "Boring answer, which is probably why the internet hates it. A repeatable bedtime and wake time beats collecting another five tiny hacks."],
  ["SS+ · TRACK WHAT REPEATS", "The useful part is noticing the pattern instead of guessing. CortiFree is where I keep the habits I actually repeat and the ones I quietly abandon."],
];
const frRankingSteps = [
  ["F · COMPLÉMENTS « CORTISOL DETOX »", "Cher ne veut pas dire utile. Si mon sommeil et ma routine sont chaotiques, ce n’est clairement pas la poudre tendance que je priorise."],
  ["D · ROUTINE PARFAITE EN 10 ÉTAPES", "Jolie sur TikTok, épuisante dans la vraie vie. Plus j’ajoute d’étapes, plus j’ai envie de tout abandonner trois jours après."],
  ["C · MAGNÉSIUM LE SOIR", "J’aime bien comme petit rituel, pas comme solution magique. Ça peut accompagner la routine, mais je ne construirais pas toute la routine autour."],
  ["B · 10 MINUTES DEHORS", "Peu d’effort et vraiment faisable les jours chargés. Je prends ça avant une nouvelle règle wellness compliquée à mémoriser."],
  ["A · TÉLÉPHONE HORS DU LIT", "Pénible au début, puis franchement libérateur. Enlever la porte d’entrée du scroll de minuit rend ma soirée beaucoup moins chaotique."],
  ["S · SOMMEIL RÉGULIER", "Réponse boring, donc forcément moins sexy sur internet. Une heure de coucher et de réveil répétable vaut mieux qu’empiler cinq mini hacks."],
...
export function createFallbackCarousel(input: CarouselGeneratorInput): CarouselSpec {
  ...
  const slides = [
    ...
    input.carouselType === "F03_ROUTINE_TIMELINE"
      ...
      : { position: input.requestedSlideCount, role: "CTA" as const, layout: input.layout, headline: input.carouselType === "F05_INTERACTIVE_CHECKLIST" ? (language === "fr" ? "Ton score n’est pas le plus important" : "Your score is not the point") : input.carouselType === "F07_RANKING" ? (language === "fr" ? "Sauvegarde pour ton prochain reset ♡" : "Save this for your next reset ♡") : (language === "fr" ? "Sauvegarde pour ton prochain reset" : "Save this for your next reset"), body: input.carouselType === "F05_INTERACTIVE_CHECKLIST" ? (language === "fr" ? "Garde les 1 ou 2 cases que tu veux changer en premier. CortiFree peut t’aider à voir ce que tu répètes." : "Keep the 1 or 2 boxes you want to change first. CortiFree can help you notice what you repeat.") : (language === "fr" ? "Choisis une seule idée et commence par là." : "Pick one idea and start there."), visualIntent: input.carouselType === "F02_EDITORIAL_COLLAGE" ? "Exactly three cohesive editorial closing images: one lifestyle hero plus two supporting details matching the carousel, natural magazine feel, no text in image" : input.carouselType === "F07_RANKING" ? "Text-first clean ivory closing frame; optional subtle feminine lifestyle support photo only, no text baked into image" : input.carouselType === "F05_INTERACTIVE_CHECKLIST" ? "One calm closing lifestyle photo with no text or UI baked into image" : "Minimal closing frame with calm background and strong save prompt", assetType: input.carouselType === "F02_EDITORIAL_COLLAGE" || input.carouselType === "F07_RANKING" || input.carouselType === "F05_INTERACTIVE_CHECKLIST" ? "stock" as const : "text_only" as const, assetQuery: input.carouselType === "F02_EDITORIAL_COLLAGE" ? "editorial wellness closing portrait details natural light" : input.carouselType === "F07_RANKING" ? "soft feminine wellness reset editorial natural light" : input.carouselType === "F05_INTERACTIVE_CHECKLIST" ? "calm realistic routine lifestyle closing natural light" : "minimal warm neutral paper texture" },
  ];
