...
describe('asset scanner', () => {
  it('indexes image metadata and classifies trailing-space folders', async () => {
    ...
    assert.equal(result.assets.length, 2); assert.equal(result.assets[0]?.source_type, 'stock'); assert.equal(result.assets[0]?.category, 'morning'); assert.equal(result.assets[0]?.width, 240); assert.equal(result.added, 1);
    const second = await scanAssets(root, result.assets); assert.equal(second.assets.length, 2); assert.equal(second.added, 0); assert.equal(second.updated, 1);
  });
});
