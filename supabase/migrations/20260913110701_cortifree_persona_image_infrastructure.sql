...
create table if not exists image_generation_jobs (
  ...
  carousel_id text,
  slide_id text,
  master_asset_id bigint not null,
  visual_reference_id text not null,
  ...
  model text,
  status text not null default 'PENDING' check (status in ('PENDING','RUNNING','DONE','FAILED','RETRY','CANCELLED')),
